...
export function Header() {
  ...
  const fetchUserData = async () => {
    try {
      const userRes = await axios.get(`http://192.168.1.102:5000/getUser/${currentUserId}`);
  ...
  const handleProfilePictureUpload = async (e) => {
    ...
    try {
      const res = await axios.post("http://192.168.1.102:5000/upload_pfp", formData, {
  ...
  const fetchCart = async () => {
    try {
      ...
      const response = await axios.get(`http://192.168.1.102:5000/view_cart/${currentUserId}`);
  ...
  const handleQuantityChange = async (productId, type) => {
    ...
    try {
      await axios.post(`http://192.168.1.102:5000${endpoint}`, {
  ...
  const handleRemoveItem = async (ordItemId) => {
    try {
      await axios.post("http://192.168.1.102:5000/remove_from_cart", {
        user_id: currentUserId,
